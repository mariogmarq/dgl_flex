...
def get_client_gradient(client_flex_model: FlexModel, client_data: Dataset):
    ...
    return data, [g.detach().clone() for g in grad(loss, model.parameters())]
    
def optimize_dummy(flex_model: FlexModel, _: Dataset, grads: List[torch.Tensor]):
    model = flex_model["model"]
    assert isinstance(model, nn.Module)
    model.to(device)
    dummy_x, dummy_y = torch.randn(1, 1, 28, 28, requires_grad=True, device=device), torch.randn(1, 10, requires_grad=True, device=device)
    ...
    def closure():
        optimizer.zero_grad()
        pred = model(dummy_x)
        loss = flex_model["criterion"](pred, dummy_y)
...
def DGL(pool: FlexPool):
    client_to_leak = pool.clients.select(1)
    pool.servers.map(copy_server_model_to_clients, client_to_leak)
    data, grads = client_to_leak.map(get_client_gradient)[0]
    pool.servers.map(optimize_dummy, grads=grads)
